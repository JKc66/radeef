...
export function MerchantIntro({ next }) {
  return <MerchantStepShell title="موّل توسّع منشأتك بالصكوك" subtitle="رديف يحوّل احتياجك التمويلي إلى صكوك يشارك فيها عملاء الإنماء." current={1}>
    <section className="merchant-cycle">
      <div><span className="icon-pill"><Icon icon={ClipboardTextIcon} /></span><p><strong>قدّم طلبك</strong><small>بيانات المنشأة واحتياج التمويل</small></p></div><i />
      <div><span className="icon-pill"><Icon icon={ShieldCheckIcon} /></span><p><strong>مراجعة الإنماء</strong><small>دراسة المنشأة والمخاطر والامتثال</small></p></div><i />
      <div><span className="icon-pill"><Icon icon={CoinsIcon} /></span><p><strong>طرح الصكوك</strong><small>جمع التمويل وصرفه وفق الشروط</small></p></div>
    </section>
    <div className="notice merchant-notice"><Icon icon={InfoIcon} /><p>تقديم الطلب لا يعني الموافقة النهائية. يحدد البنك الهيكلة والعائد والمدة بعد اكتمال الدراسة.</p></div>
...
export function MerchantDetails({ data, update, next }) {
  ...
  return <MerchantStepShell title="عرّفنا على منشأتك" subtitle="نحتاج معلومات أولية لنكوّن صورة واضحة عن فرصة التمويل." current={2}>
    ...
    <div className="field"><label htmlFor="business-name">{data.entityType === "idea" ? "اسم الفكرة أو المشروع" : "اسم المنشأة"}</label><div className="input-with-icon"><Icon icon={StorefrontIcon} /><input id="business-name" value={data.businessName} onChange={e => update({ businessName: e.target.value })} placeholder="مثال: شركة مذاق القهوة" /></div></div>
    ...
    <div className="document-row"><Icon icon={FileTextIcon} /><p><strong>المستندات</strong><small>سيطلبها فريق رديف بعد التقييم الأولي</small></p><span>لاحقًا</span></div>
...
export function MerchantPlan({ data, next }) {
  ...
  return <MerchantStepShell title="هيكلة أولية للصك" subtitle="تصور مبدئي يساعدك على فهم طريقة تمويل الطلب عبر رديف." current={3}>
    <section className="merchant-plan-card dark-card">
      <div className="plan-top"><span><Icon icon={BuildingsIcon} /></span><p><small>{data.sector}</small><strong>{data.businessName}</strong></p><em>تصور أولي</em></div>
    ...
    <section className="bank-role info-row"><Icon icon={ShieldCheckIcon} /><p><strong>دور بنك الإنماء</strong><span>يدرس الطلب، يصنّف المخاطر، يعتمد الهيكلة، وينظم طرح الصكوك وتوزيع العوائد.</span></p></section>
...
export function MerchantReview({ data, update, next }) {
  return <MerchantStepShell title="راجع طلب التمويل" subtitle="بعد الإرسال ينتقل طلبك إلى فريق رديف للدراسة الأولية." current={4}>
    ...
    <section className="review-timeline"><div><Icon icon={ClipboardTextIcon} /><span><strong>اليوم</strong><small>استلام الطلب</small></span></div><i /><div><Icon icon={ClockIcon} /><span><strong>المرحلة التالية</strong><small>مراجعة أولية وتواصل الفريق</small></span></div><i /><div><Icon icon={HandshakeIcon} /><span><strong>بعد الموافقة</strong><small>العقود وطرح الصكوك</small></span></div></section>
...
export function MerchantSuccess({ data, onHome }) {
  return <main className="success-screen merchant-success screen"><div className="success-symbol"><Icon icon={CheckCircleIcon} size={58} weight="fill" /></div><img src={logo} alt="رديف" className="success-logo" /><h1>وصلنا طلبك</h1><p>طلب تمويل {data.businessName} أصبح الآن لدى فريق رديف للمراجعة الأولية.</p><div className="request-card"><span>رقم الطلب</span><strong>RD-2026-1048</strong><em><Icon icon={ClockIcon} size={16} /> قيد المراجعة</em></div><section className="success-path merchant-path"><span><Icon icon={ClipboardTextIcon} /><b>استلام</b></span><Icon icon={ArrowRightIcon} /><span><Icon icon={ShieldCheckIcon} /><b>دراسة</b></span><Icon icon={ArrowRightIcon} /><span><Icon icon={HandshakeIcon} /><b>اعتماد</b></span></section><PrimaryButton onClick={onHome}>العودة إلى رديف</PrimaryButton></main>;
}

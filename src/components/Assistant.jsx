...
export function Assistant({ context, onClose }) {
  ...
  return <section className="assistant-overlay" aria-label="مساعد رديف">
    <header className="assistant-header"><button type="button" className="icon-button" onClick={onClose} aria-label="إغلاق المساعد"><Icon icon={ArrowRightIcon} /></button><div><span><Icon icon={RobotIcon} /></span><p><strong>مساعد رديف</strong><small>متصل الآن</small></p></div><img src={logo} alt="رديف" /></header>
    <div className="assistant-insight dark-insight"><Icon icon={SparkleIcon} /><p><strong>{context === "merchant" ? "مساعدة في طلب التمويل" : "قراءة استباقية لهدفك"}</strong><span>{context === "merchant" ? "اسأل عن الدراسة، الهيكلة، العقود، أو صرف التمويل." : "استمرارك على نفس المعدل قد يضيف 1,280 ريال خلال سنة قبل العوائد."}</span></p></div>
    <div className="chat-messages scroll-hide" aria-live="polite">{messages.map(message => <div className={`chat-message ${message.from}`} key={message.id}><span>{message.from === "bot" && <Icon icon={RobotIcon} size={16} />}{message.text}</span></div>)}{typing && <div className="chat-message bot typing"><span><i /><i /><i /></span></div>}</div>
    ...
    <form className="chat-composer" onSubmit={event => { event.preventDefault(); ask(draft); }}><label htmlFor="assistant-message">اكتب سؤالك</label><div><input id="assistant-message" value={draft} onChange={event => setDraft(event.target.value)} placeholder="اسأل رديف..." /><button type="submit" aria-label="إرسال" disabled={!draft.trim() || typing}><Icon icon={PaperPlaneRightIcon} weight="fill" /></button></div></form>
    <small className="assistant-disclaimer"><Icon icon={InfoIcon} size={14} /> إجابات توعوية وليست توصية استثمارية أو موافقة تمويل.</small>
  </section>;
}

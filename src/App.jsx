    ...
    <>
      ...
      <header className="app-header">
        <div className="header-actions">
          <button type="button" className="icon-button coral" aria-label="تسجيل الخروج"><Icon icon={ArrowLeft} /></button>
          <button type="button" className="icon-button" aria-label="التنبيهات"><Icon icon={Bell} /></button>
          <button type="button" className="icon-button" aria-label="المساعد الذكي" onClick={() => onScreen("assistant")}><Icon icon={PencilSimple} /></button>
        </div>
        <button type="button" className="profile" onClick={() => onScreen("start")} aria-label="العودة إلى شاشة رَديف">
          <div><b>عبدالله</b><small>10 <span>✣</span></small></div><strong>ع ع</strong>
        </button>
      </header>
      {screen !== "start" && screen !== "setup" && <button type="button" className="mini-title" onClick={() => onScreen("start")}>{labels[screen]}</button>}
    </>
  );
}
...
function BottomNav({ active, onScreen }) {
  return <nav className="bottom-nav" aria-label="التنقل الرئيسي">{bottomNavItems.map(([label, icon, id, target]) => {
    const isActive = active === id;
    return <button type="button" key={label} className={isActive ? "nav-active" : ""} aria-current={isActive ? "page" : undefined} onClick={() => onScreen(target)}><Icon icon={icon} size={27} /><span>{label}</span></button>;
  })}</nav>;
}
...
function Start({ onScreen }) {
  return <div className="screen start-screen">
    <section className="hero-card">
      ...
      <button type="button" className="primary" onClick={() => onScreen("setup")}>تفعيل الخدمة</button>
...
function Setup({ onScreen }) {
  ...
  return <div className="screen setup-screen"><ScreenTitle title="إعداد رَديف" subtitle="فعل الخدمة خلال خطوات بسيطة" />
    <div className="steps"><span className="active">1</span><i></i><span>2</span><i></i><span>3</span><label htmlFor="step-account">الحساب</label><label htmlFor="step-rounding">قاعدة التقريب</label><label htmlFor="step-goal">الهدف</label></div>
    <section className="card form-card"><h2>1. الحساب</h2><p>اختر الحساب الذي تريد تفعيل رَديف عليه</p><button type="button" className="choice selected"><Icon icon={Wallet} /> الحساب الجاري ••••7000 <span><Icon icon={CheckCircle} /></span><small>الرصيد الحالي <em>1,245.40</em> ريال</small></button></section>
    <section className="card form-card"><h2>2. قاعدة التقريب</h2><p>اختر طريقة التقريب المناسبة لك</p><div className="choice-grid">{["1","5","10"].map(value => <button type="button" className={round === value ? "choice selected" : "choice"} onClick={() => setRound(value)} key={value}>أقرب {value} ريال {round === value && <Icon icon={CheckCircle} />}</button>)}</div></section>
    <section className="card form-card"><h2>3. الهدف</h2><p>اختر وجهة مدخراتك من رَديف</p><div className="goal-grid">{[["save","الادخار الذكي",Target],["invest","الاستثمار",ChartBar],["local","دعم منشآت محلية",Buildings]].map(([value,label,icon]) => <button type="button" onClick={() => setGoal(value)} className={goal === value ? "goal selected" : "goal"} key={value}><Icon icon={icon} /><span>{label}</span>{goal === value && <Icon icon={CheckCircle} />}</button>)}</div></section>
    ...
    <button type="button" className="primary" onClick={() => onScreen("activity")}>متابعة</button><button type="button" className="outline" onClick={() => onScreen("start")}>لاحقًا</button>
  </div>;
}
...
function Activity({ onScreen }) { return <div className="screen"><ScreenTitle title="اجمع فكتك صمتًا" subtitle="كل عملية شراء تساهم تلقائيًا في رَديف." />
  <div className="filter"><button type="button" className="selected">اليوم</button><button type="button">هذا الأسبوع</button><button type="button">الكل</button></div>
  ...
  <section className="card tx-list">{transactions.map(([name,price,round,icon]) => <button type="button" key={name} onClick={() => onScreen("reached")}><Icon icon={icon} className="coral"/><b>{name}</b><span>{price}</span><em>{round}<small> إلى رَديف</small></em></button>)}</section>
...
function Reached({ onScreen }) { return <div className="screen"><ScreenTitle title="الاستثمار التلقائي" art={walletArt} />
  ...
  <button type="button" className="primary" onClick={() => onScreen("portfolio")}>عرض المحفظة</button><button type="button" className="text-button" onClick={() => onScreen("start")}>العودة إلى رَديف <Icon icon={ArrowLeft}/></button>
</div>; }
...
function Assistant() { const [message, setMessage] = useState("كيف أصل لهدفي أسرع؟"); return <div className="screen assistant-screen"><ScreenTitle title="المساعد الذكي" />
  <section className="card assistant-hero"><img src={robotArt} alt="روبوت مساعد"/><div><span className="tag">توصية استباقية <Icon icon={Sparkle}/></span><p>لاحظنا صرفك المتكرر في المقاهي؛ قد يناسبك الاستثمار في قطاع الأغذية والمشاريع المحلية المرتبطة به.</p></div><div className="quick-actions"><button type="button">رفع التقريب <Icon icon={SlidersHorizontal}/></button><button type="button">اقتراح قطاع <Icon icon={ChartBar}/></button></div></section>
  <section className="card chat"><h2>المحادثة <Icon icon={Receipt} className="coral"/></h2><div className="user-bubble">{message}<small>9:40 م</small></div><div className="bot-row"><Icon icon={Robot} size={38}/><div className="bot-bubble">يمكنك رفع قاعدة التقريب إلى أقرب 5 ريالات وتحديد حد شهري 150 ريال لزيادة النمو بشكل متوازن.<small>9:41 م</small></div></div><div className="suggestions"><button type="button" onClick={() => setMessage("غيّر الإعدادات")}>غيّر الإعدادات <Icon icon={Gear}/></button><button type="button" onClick={() => setMessage("اعرض هدفي")}>اعرض هدفي <Icon icon={Target}/></button><button type="button" onClick={() => setMessage("المزيد من النصائح")}>المزيد من النصائح <Icon icon={Lightbulb}/></button></div></section>
...
function Portfolio({ onScreen }) { return <div className="screen"><ScreenTitle title="هذا أثر هلالتك" subtitle="استثمارك اليوم يصنع الفرق غدًا." art={coinArt}/>
  ...
  <div className="metric-cards"><button type="button"><Icon icon={ChartBar} className="coral"/>الأرباح <b className="green">+240</b><small>ريال</small></button><button type="button"><Icon icon={Wallet} className="coral"/>إجمالي المستثمر <b>3,320</b><small>ريال</small></button><button type="button"><Icon icon={Coins} className="coral"/>الفكة المتجمعة <b>1,860</b><small>ريال</small></button></div>
  ...
  <button type="button" className="card continue-card" onClick={() => onScreen("impact")}><img src={coinArt} alt=""/><span><b>استمرارك في رَديف</b>ساعدك على بناء استثمار تدريجي بدون ضغط مالي.</span></button>
</div>; }
...
const discoverRows = [["تموينات الحي","27.40 ر.س","+0.60",ShoppingBag],["قهوة الصباح","14.50 ر.س","+0.50",Coffee],["محطة الوقود","49.10 ر.س","+0.90",GasPump],["صيدلية","31.80 ر.س","+0.20",CheckCircle],["توصيل","18.40 ر.س","+0.60",Truck]];

function Discover({ onScreen }) { const [filter,setFilter]=useState("مستقطعة"); return <div className="screen discover"><ScreenTitle title="كشف الحركات" />
  <div className="filter">{["الكل","مستقطعة","غير مؤهلة"].map(x=><button type="button" className={filter===x?"selected":""} onClick={()=>setFilter(x)} key={x}>{x}</button>)}</div>
  ...
  <section className="discover-list">{discoverRows.map(([name,price,amount,icon])=><button type="button" className="card" onClick={()=>onScreen("setup")} key={name}><span className="tile"><Icon icon={icon}/></span><span><b>{name}</b><small>اليوم 10:30 ص</small></span><strong>{price}</strong><em><Icon icon={Sparkle} size={17}/> {amount} رَديف</em></button>)}</section>
  <button type="button" className="card info-button" onClick={()=>onScreen("setup")}><Icon icon={CheckCircle} className="coral"/> يمكنك تعديل قواعد التقريب في أي وقت <Icon icon={ArrowLeft} className="coral"/></button>
</div>; }
